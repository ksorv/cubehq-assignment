...
const Sidebar: FC = () => {
    ...
    const fetcher = useCallback(async (paginationParams?: PaginationParams | null) => {
    ...
    const merger = useCallback((oldData: Array<Customer> | null, newData: Array<Customer>) => {
        if (!oldData) return newData

        return [...oldData, ...newData]
    }, [])

    const {data, loading, fetching, error, loadMore} = usePaginatedFetcher(fetcher, merger);
    const listRef = useInfiniteScroll(loadMore, 4)

    if (loading) {
        return <div className={classnames(styles.container, styles.loading)}><Loader text="Fetching customers..."/>
        </div>
    }

    if (error) {
        return <div className={classnames(styles.container, styles.error)}>{error}</div>
    }
        ...
        <div className={styles.container} ref={listRef}>
            {data?.map((customer) => (
                <Link key={customer.id}
                      ...
                      className={classNames(styles.customer, selectedCustomerId === customer.id && styles.selected)}>
                    <img src={customer.avatar} className={styles.avatar} alt={customer.name}/>
            ...
            {fetching && <div className={styles.fetchingMore}><Loader/></div>}
        </div>
    );
}
